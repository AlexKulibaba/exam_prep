...
```{r}
ar2 <- arima.sim(model=list(ar=c(.9,-.5)), n=4000)
...
-   Check if the series is stationary and if it is not, difference it until you obtain a stationary time series (done in previous lab).
    ...
    cat_diff3 <- ts(diff(cat_diff2), start = 1977, frequency = 4)
    ...
    ```{r}
    library(tseries)
    kpss.test(cat_diff3) #if p value ca >0.05 then stationary
    ```

    KPSS test says all are stationary, but big spike at the end isnt. Only in diff3 it seems rather stationary, because spike is not as high and goes in both directions ==\> mean doesn't change
...
-   Draw the PACF correlogram to select a candidate order of an AR model for the stationary time series obtained in the previous section. Then, use the `auto.arima` function to select $p$ automatically and fit a suitable AR model. What can you conclude?
    ...
    we should take AR(2), as values are above confidence interval until lag 2

    ```{r}
    model.ma <- auto.arima(cat_diff3, max.q=0, max.p=2, d=0, seasonal=FALSE)
    model.ma
    ```

    auto arima concludes the same
...
```{r}
acf(cat_diff3)
model.ma <- auto.arima(cat_diff3, max.q=5, max.p=0, d=0, seasonal=FALSE)
model.ma
```
...
```{r}
model.ma <- auto.arima(cat_diff3, max.q=5, max.p=5, d=0, seasonal=FALSE)
model.ma
```
